...
Canonically lives in `host/` here; the live deployments are symlinks into it
(`./host/install.sh` does the wiring, shell.json registration is manual):

- `host/sofle-layerd` (systemd user unit `host/sofle-layer.service`, enabled):
  finds the board's QMK debug console hidraw by parsing report descriptors
  (usage page 0xff31, usage 0x74 — the usage_page sysfs files don't exist)
  and mirrors `sofle layer=N` lines to `$XDG_RUNTIME_DIR/sofle-layer`.
- `host/plugin/` — omarchy shell plugin (live indicator + layer-diagram popup),
  symlinked to `~/.config/omarchy/plugins/alvarosaavedra.sofle-layer/`.
  Edits to Panel.qml may be served stale from the QML document cache —
  `omarchy restart shell` flushes it.
- `~/.local/share/sofle/layers.json` — diagram data, regenerated by
  `tools/gen-layout-json.py` (install script runs it).
- Flashing rule: firmware console output only works on the USB-connected half;
  layer state from the other half arrives over TRRS via split sync.
